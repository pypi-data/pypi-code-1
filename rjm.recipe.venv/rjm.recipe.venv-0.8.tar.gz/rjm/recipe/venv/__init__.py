...
class Venv(object):
    ...
    def install(self):
        ...
        # extra virtualenv options?
        venv_options = ''
        if self.options.get('venv_options'):
        ...
        if not is_venv:
            ...
            call_list = [sys.executable, venv_file]
            if venv_options:
                call_list.extend(venv_options)
            call_list.append(buildout_dir)
            subprocess.call(call_list)

        buildout_path = '%s/buildout' % bin_dir
        buildout_args = sys.argv[1:]

        # re-run bootstrap?
        buildout_exec = open(buildout_path)
        is_venv_bootstrapped = venv_python_path in buildout_exec.readline()
        if not is_venv_bootstrapped:
            # we might need to replicate a -c option
            dash_c_args = []
            for i, arg in enumerate(buildout_args):
                if arg == '-c':
                    dash_c_args.extend([arg, buildout_args[i+1]])
                    break
                elif arg.startswith('-c'):
                    dash_c_args = [arg]
                    break

            # perform the new bootstrap...
            call_list = []
            bootstrap_path = '%s/bootstrap.py' % buildout_dir
            if os.path.exists(bootstrap_path):
                # ...using bootstrap.py
                call_list = [venv_python_path, bootstrap_path]
                call_list.extend(dash_c_args)
            else:
                # ...using 'buildout bootstrap'
                call_list = [venv_python_path, buildout_path]
                call_list.extend(dash_c_args)
                call_list.append('bootstrap')
            subprocess.call(call_list)

        # re-launch the buildout?
        if sys.executable != venv_python_path:
            call_list = [buildout_path]
            # re-use any buildout call arguments
            call_list.extend(buildout_args)
            subprocess.call(call_list)
            sys.exit()
