...
class Venv(object):
    ...
    def install(self):
        ...
        if self.options.get('no_site_packages'):
            cfgval = self.options.get('no_site_packages')
            cfgval = cfgval.strip().lower()
            if cfgval not in ['false', 'f', 'no', 'n',  '0']:
        ...
        if not is_venv:
            ...
            if no_site_packages:
                call_list.append('--no-site-packages')
